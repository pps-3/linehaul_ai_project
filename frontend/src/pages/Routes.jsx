...
// Exactly this many routes are shown on one page.
const PAGE_SIZE = 6
...
export default function RoutesPage({ notify }) {
  ...
  const unassigned = orders.filter(
    (order) =>
      !order.routeId &&
      ['READY', 'CREATED', 'DRAFT'].includes(
        String(order.status).toUpperCase()
      )
  )

  const driverName = (id) =>
    drivers.find((d) => d.driverId === id)?.name

  const truckType = (id) =>
    vehicles.find((v) => v.truckId === id)?.type
  ...
  async function dropOrder(route, order) {
    try {
      const updated = await assignOrderToRoute(
        route.routeId,
        order.orderId
      )

      notify(
        `Order ${order.orderId} assigned to route ${route.routeId}.`,
        'success'
      )
  ...
  async function openDetails(route) {
    ...
    try {
      setRouteOrders(
        await getRouteOrders(route.routeId)
      )
    } catch {
      setRouteOrders([])
    }
  ...
  async function refreshViewing(routeId) {
    const fresh = (await getRoutes()).find(
      (r) => r.routeId === routeId
    )
  ...
  async function release(kind, route) {
    try {
      if (kind === 'truck') {
        ...
        notify(
          'Truck removed from route.',
          'success'
        )
      } else {
        await unassignDriver(route.routeId)
        notify(
          'Driver removed from route.',
          'success'
        )
      }
  ...
  async function dispatch(route) {
    try {
      ...
      notify(
        `Route ${route.routeId} dispatched.`,
        'success'
      )
  ...
  async function removeOrder(route, order) {
    try {
      await removeOrderFromRoute(
        route.routeId,
        order.orderId
      )

      notify(
        `Order ${order.orderId} removed from route.`,
        'success'
      )
  ...
  const availableTrucks = vehicles.filter(
    (v) =>
      !v.routeId &&
      v.status === 'AVAILABLE'
  )
      ...
      <div className="page-head">
        <div>
          ...
          <p className="page-sub">
            Drag an order from the left onto a route to
            assign it. Capacity, ETA and readiness update
            automatically.
          </p>
        </div>
      ...
      >
        <div className="pile">
          <div className="pile-head">
            ...
            <small>
              {unassigned.length} waiting for a route -
              drag one to the right
            </small>
          </div>
        ...
        <div className="route-grid">
          {pageRoutes.map((route) => {
            const editable = EDITABLE.includes(
            ...
            return (
              <div
                key={route.routeId}
                className={`route-card drop-zone${
                  overRoute === route.routeId
                    ? ' over'
                    : ''
                }`}
              ...
              >
                ...
                <ProgressBar
                  percent={route.capacityPercent}
                />

                <div className="progress-label">
                  <span>
                    {route.currentWeight.toLocaleString()} /
                    {` ${route.maxCapacity.toLocaleString()}`} lbs
                  </span>

                  <span>
                    {route.capacityPercent}% Capacity
                  </span>
                </div>
