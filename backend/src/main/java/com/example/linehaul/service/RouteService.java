...
public class RouteService {
    ...
    public Route update(String routeId, Route changes) {
        ...
        // Departure or duration changed -> the ETA changes too.
        route.setEta(LinehaulUtil.calculateEta(route.getDepartureTime(), route.getTravelDuration()));
        // Every order on the route arrives with the truck.
        for (String orderId : route.getOrderIds()) {
    ...
    // ------------------------------------------------------------------
    // assign / unassign an order  (also used by drag and drop)
    // ------------------------------------------------------------------
    ...
    public Route assignOrder(String routeId, String orderId) {
        ...
        // An order can only belong to one route.
        ...
        // Capacity check - nothing is saved if this fails.
        ...
        // Recalculate the ETA before copying it onto the order, so the order
        // always arrives with the truck.
        route.setEta(LinehaulUtil.calculateEta(route.getDepartureTime(), route.getTravelDuration()));
        ...
        // The route may now be ready (or blocked) - recompute its status.
    ...
    public Route assignTruck(String routeId, String truckId) {
        ...
        // Null-safe: a brand new truck has no route yet.
    ...
    public Route assignDriver(String routeId, String driverId) {
        ...
        // Null-safe: a brand new driver has no route yet.
    ...
    private Route refreshStatus(Route route) {
        ...
        // A route that is already on the road keeps its own status.
        String status = route.getStatus() == null ? "" : route.getStatus().toUpperCase();
        if (status.equals(Status.DISPATCHED) || status.equals(Status.IN_TRANSIT)
                || status.equals(Status.COMPLETED)) {
            return route;
